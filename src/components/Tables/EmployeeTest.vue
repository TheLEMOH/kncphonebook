    ...
    <template #columns>
      <el-table-column type="expand">
        <template #default="props">
          <el-table :data="props.row.employees">
            <el-table-column prop="room" label="Кабинет" width="90px"> </el-table-column>
            <el-table-column prop="name" label="ФИО" min-width="150"> </el-table-column>
            <el-table-column prop="position.name" label="Должность" min-width="100px"> </el-table-column>
            <el-table-column prop="organization.shortName" label="Организация" width="125px"> </el-table-column>
            <el-table-column prop="division.name" label="Структ. подразделение"> </el-table-column>
            <el-table-column prop="subdivision.name" label="Подразделение"> </el-table-column>
            <el-table-column prop="phone" label="Рабочий тел." width="115px"> </el-table-column>
            <el-table-column prop="email" label="Почта" min-width="150"> </el-table-column>
          </el-table>
        </template>
      </el-table-column>
      <el-table-column label="Кабинет" width="80px">
        <template #header>
...
const data = ref(await Get(`/employees/pages/group`));
...
const SearchGet = async (page = 1) => {
  ...
  const res = await Get(`/employees/pages/group?${searchParams}`);
  data.value = res;
};
